...
def ConnectmainDB(ip, pw, id) :
    try : return pymysql.connect(ip, user=id, password=pw, database='youtuberDB', charset='utf8mb4')
    except : return None

# 사용 가능
# admin 용 : 주어진 pw로 본인의 DB에 접속한다.
def ConnectDB(pw, id) :
    try : return pymysql.connect(host='127.0.0.1', user=id, password=pw)
    except : return None
...
def newDB(pw, id):
    con = pymysql.connect(host='127.0.0.1', user=id, password=pw)
    cur = con.cursor()
    try :
