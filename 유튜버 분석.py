...
# 프로그램 설명 : 유튜브에 업로드된 정보를 기반으로 유튜버를 분석한다.
...
# * 해당 코드는 검색한 유튜버 한명의 정보만 수집한다. 추후에 여러명의 유튜버들의 정보를 수집하고 DB에 저장하도록 코드를 개선하면 좋을 것 같다.
# * 수집한 정보를 통계적으로 분석하고 분석결과를 반환하는 코드를 추가할 필요가 있다.
...
# 자료구조 설명명# youtubers_data = {'name':[], 'videos':[]} 아직은 해당 코드에서 쓰이지 않는 자료구조
videos_data = {'name':[], 'thumbnail':[], 'view':[], 'previous_time':[], 'video_url':[], 'start_date':[],
'comment_num':[], 'likes_num':[], 'unlikes_num':[], 'comments':[]} # 최종적으로 모든 데이터는 여기에 저장됨
...
browser.find_element_by_xpath('//*[@class="scrollable style-scope paper-tabs"]/paper-tab[2]').click() # '동영상' 카테고리 클릭
...
list_videoList_element = VideoList_element0.find_all('ytd-grid-video-renderer', {'class': 'style-scope ytd-grid-renderer'}) # 각각의 영상의 정보가 담긴 element들의 리스트 (영상 1개당 value 1개)

for each_VideoList_element in list_videoList_element:
    # 영상으로 이동
    video_url = 'https://www.youtube.com' + each_VideoList_element.find('a', {'id': 'thumbnail'})['href'] # 영상 url이 유튜브 기본 url을 포함하지 않기 때문에 기본 url을 앞에 붙임
    ...
    videos_data['view'] += [meta.find_all('span', {'class': 'style-scope ytd-grid-video-renderer'})[0].text.split()[1]]
    videos_data['previous_time'] += [meta.find_all('span', {'class': 'style-scope ytd-grid-video-renderer'})[1].text]
    videos_data['start_date'] += [video_html.find('span', {'class': 'date style-scope ytd-video-secondary-info-renderer'}).text]
    videos_data['comment_num'] += [video_html.find('h2',{'id':'count'}).find('yt-formatted-string').text]
    videos_data['likes_num'] += [video_html.find('yt-formatted-string',{'id': 'text', 'class': 'style-scope ytd-toggle-button-renderer style-text','aria-label': re.compile('좋아요')}).text+'개']
    videos_data['unlikes_num'] += [video_html.find('yt-formatted-string',{'id': 'text', 'class': 'style-scope ytd-toggle-button-renderer style-text','aria-label': re.compile('싫어요')}).text+'개']
    for each_comment_element in list_comment_element:
        comments_data['youtube_id'] += ["".join(re.findall('[가-힣0-9a-zA-Z]', each_comment_element.find('a', {'id': 'author-text'}).find('span').text))]
        comments_data['comment'] += [each_comment_element.find('yt-formatted-string',{'id':'content-text','class':'style-scope ytd-comment-renderer'}).text]
        try:
            comments_data['like_num'] += ["".join(re.findall('[0-9]', each_comment_element.find('span', {'id': 'vote-count-left'}).text)) + '개']
        except:
